...
import getUniformDistValues from "@/lib/getUniformDistValues";
import getUniformSimValues from "@/lib/getUniformSimValues";
...
export default async function UniformResults() {
  ...
  if (!distMin || !distMax || !simPeriodsPerYear) {
    router.push("/");
  }

  const { distValues } = await getUniformDistValues(distMin, distMax);
  const {
    simValues,
    simMin,
    simMax,
    ...
    lowerCI,
    upperCI,
  } = await getUniformSimValues(distMin, distMax, simPeriodsPerYear);
